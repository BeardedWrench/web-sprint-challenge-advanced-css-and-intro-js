...
function getArtistByIndex( arr, index ) {
  let info = arr[ index ];
  return `the artist at index ${index} is ${info.name}`;
}  
...
// setup of object: "years": "1471 - 1528"

function get20s( arr ){
  let art = [];
  for( let i = 0; i < arr.length; i++ ){
    let word = arr[ i ].years.split( ' ' );
    if( word[ 0 ] >= 1900 && word[ 0 ] < 2000 ){
      art.push( arr[ i ].name );
    }
  }
  return art;
...
function addArtist( arr ){
  ...
  let newObj = { 
    ...
    genre: 'Web Design', 
    nationality: 'Irish',
    bio: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer at lectus porttitor, lobortis urna ut, imperdiet ligula. Suspendisse non porttitor nunc, fermentum tempus nulla. Aenean nisi sem, mollis quis tellus non, consectetur mattis lacus. Mauris at nisl diam.'
  }
